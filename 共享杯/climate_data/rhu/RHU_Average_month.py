...
columns_TEM_name = ["区站号", "纬度", "经度", "观测场拔海高度", "年", "月", "日", "平均相对湿度", "最小相对湿度(仅自记)", "平均相对湿度质量控制码", "最小相对湿度质量控制码"]


def data_process(group_line):
    bool_index = (group_line["平均相对湿度"] != 32766)
    line_index = np.where(bool_index)[0]
    group_line["平均相对湿度"] = group_line["平均相对湿度"].iloc[line_index].mean() * 0.01
    group_line.rename({"平均相对湿度": "月平均相对湿度"})
    return pd.DataFrame([group_line.iloc[0, :]], columns=group_line.columns)


filename_list = os.listdir(os.getcwd())
filename_list.remove("RHU_Average_month.py")
base_dir = os.getcwd()
for filename in filename_list:
    ...
    data = raw_data[["区站号", "纬度", "经度", "观测场拔海高度", "年", "月", "平均相对湿度"]]
