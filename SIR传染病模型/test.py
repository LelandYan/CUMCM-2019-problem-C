...
Updates_NC = pd.read_csv("Updates_NC.csv", encoding="gbk")
...
class preProcess:
    def __init__(self):
        ...
        self.wuHan = {'报道时间': wuHanInfection.index, '新增确诊': wuHanInfection.values, '新增出院': wuHanRecovered.values,
    ...
    def getTotal(self):
        ...
        self.wuHan = self.wuHan.join(
            pd.DataFrame([wuHanTotalInfection, wuHanTotalRecovered, wuHanTotalDead], index=['累计确诊', '累计出院', '累计死亡']).T)
        print(self.wuHan)
    ...
    def report(self):
        ...
        plt.xlabel('Day')
        plt.ylabel('Number of infection people(Wu Han)')
        plt.show()
...
class estimationInfectionProb:
    ...
    def costFunction(self, infectionProb):
        ...
        res = np.array(np.exp((infectionProb * self.nContact - self.gamma) * self.timeRange) - infectionData.wuHan.loc[
            self.timeRange - self.dataStartTimeStep, '累计确诊'])
        return (res ** 2).sum() / self.timeRange.size

    def optimize(self):
        self.solution = minimize(self.costFunction, np.array(self.x0), method='nelder-mead',
